...
local GetItemInfoInstant = _G.GetItemInfoInstant or _G.C_Item.GetItemInfoInstant
local issecretvalue = _G.issecretvalue or function() return false end
-- LE_ITEM_QUALITY_* globals were removed on modern clients
local LE_ITEM_QUALITY_POOR = _G.LE_ITEM_QUALITY_POOR or 0
local LE_ITEM_QUALITY_COMMON = _G.LE_ITEM_QUALITY_COMMON or 1
local LE_ITEM_QUALITY_UNCOMMON = _G.LE_ITEM_QUALITY_UNCOMMON or 2
local LE_ITEM_QUALITY_RARE = _G.LE_ITEM_QUALITY_RARE or 3
local LE_ITEM_QUALITY_EPIC = _G.LE_ITEM_QUALITY_EPIC or 4
local LE_ITEM_QUALITY_LEGENDARY = _G.LE_ITEM_QUALITY_LEGENDARY or 5
...
-- ITEM_QUALITY_COLORS can still be empty while addons load on modern clients
local function QualityColor(quality)
  local c = ITEM_QUALITY_COLORS and ITEM_QUALITY_COLORS[quality]
  if c and c.color then return c.color end
  if C_ColorOverrides and C_ColorOverrides.GetColorForQuality then
    local color = C_ColorOverrides.GetColorForQuality(quality)
    if color then return color end
  end
  local r, g, b = (_G.GetItemQualityColor or C_Item.GetItemQualityColor)(quality)
  return CreateColor(r or 1, g or 1, b or 1)
end
...
elseif WOW_PROJECT_ID == WOW_PROJECT_CLASSIC then
  BRACKET_SIZE = 200
elseif not MAX_PLAYER_LEVEL_TABLE then
  -- WoW Forever: vanilla content on a modern (mainline) client
  BRACKET_SIZE = 200
end
local MAX_PLAYER_LEVEL = (MAX_PLAYER_LEVEL_TABLE and LE_EXPANSION_LEVEL_CURRENT and MAX_PLAYER_LEVEL_TABLE[LE_EXPANSION_LEVEL_CURRENT])
  or (GetMaxLevelForPlayerExpansion and GetMaxLevelForPlayerExpansion()) or 60
...
local colorPoor = QualityColor(LE_ITEM_QUALITY_POOR)
local gradientColors = {
  QualityColor(LE_ITEM_QUALITY_UNCOMMON),
  QualityColor(LE_ITEM_QUALITY_RARE),
  QualityColor(LE_ITEM_QUALITY_EPIC),
  QualityColor(LE_ITEM_QUALITY_LEGENDARY),
}
...
function lib:UNIT_INVENTORY_CHANGED(event,...)
  local unit = ...
  if unit and not issecretvalue(unit) and UnitIsPlayer(unit) then
