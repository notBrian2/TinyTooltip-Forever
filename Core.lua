...
-- tooltips (skip ones that don't exist on this client, so ipairs sees them all)
addon.tooltips = {}
for _, name in ipairs({"GameTooltip", "ItemRefTooltip", "ShoppingTooltip1", "ShoppingTooltip2", "WorldMapTooltip",
                       "ItemRefShoppingTooltip1", "ItemRefShoppingTooltip2", "NamePlateTooltip"}) do
    if (_G[name]) then tinsert(addon.tooltips, _G[name]) end
end
...
function addon:GetQuestBossIcon(unit)
    if nosecret(UnitIsQuestBoss(unit)) then
...
function addon:GetPVPIcon(unit)
    if (nosecret(UnitIsPVPFreeForAll(unit))) then
...
function addon:GetRoleIcon(unit)
    local role = nosecret(UnitGroupRolesAssigned(unit))
    if (role) then
...
function addon:GetRaidIcon(unit)
    local index = nosecret(GetRaidTargetIndex(unit))
...
function addon:GetClassIcon(class)
    if (not class or not CLASS_ICON_TCOORDS[strupper(class)]) then return end
...
function addon:GetFriendIcon(unit)
    if (UnitIsPlayer(unit)) then
        local guid = nosecret(UnitGUID(unit))
        if (guid and C_FriendList and nosecret(C_FriendList.IsFriend(guid))) then
...
function addon:GetUnitSpeed(unit)
    ...
    if (compat.anysecret(speed, flightSpeed, swimSpeed)) then return end
    if (not speed or speed == 0) then return end
    speed = speed/BASE_MOVEMENT_SPEED*100
    swimSpeed = swimSpeed/BASE_MOVEMENT_SPEED*100
	flightSpeed = flightSpeed/BASE_MOVEMENT_SPEED*100
	if (UnitIsOtherPlayersPet(unit)) then
    elseif (nosecret(IsSwimming(unit))) then
		speed = swimSpeed
	elseif (nosecret(IsFlying(unit))) then
		speed = flightSpeed
	end
    return speed+0.5
end
...
-- True when the game hides this unit's identity from addons (secret values).
-- TinyTooltip then leaves Blizzard's default tooltip untouched.
function addon:IsUnitRestricted(unit)
    if (not unit or issecret(unit)) then return true end
    return compat.anysecret(UnitName(unit)) or compat.anysecret(UnitClass(unit))
        or compat.anysecret(UnitRace(unit)) or issecret(UnitSex(unit))
        or issecret(UnitGUID(unit)) or issecret(UnitPVPName(unit))
        or issecret(UnitCreatureType(unit)) or issecret(UnitIsPlayer(unit))
        or issecret(UnitLevel(unit)) or issecret(UnitReaction(unit, "player"))
end
...
function addon:GetUnitInfo(unit)
    ...
    t.statusAFK    = nosecret(UnitIsAFK(unit)) and AFK
    t.statusDND    = nosecret(UnitIsDND(unit)) and DND
...
LibEvent:attachTrigger("tooltip.style.font.header", function(self, frame, fontObject, fontSize, fontFlag)
    ...
    GameTooltipHeaderText:SetFont(font, size, compat.FontFlag(flag))
end)
...
LibEvent:attachTrigger("tooltip.style.font.body", function(self, frame, fontObject, fontSize, fontFlag)
    ...
    GameTooltipText:SetFont(font, size, compat.FontFlag(flag))
end)
...
LibEvent:attachTrigger("tooltip.statusbar.font", function(self, font, size, flag)
    ...
    if (font ~= origFont or size ~= origSize or flag ~= origFlag) then
        GameTooltipStatusBar.TextString:SetFont(font or origFont, size or origSize, compat.FontFlag(flag or origFlag))
    end
end)
...
LibEvent:attachTrigger("tooltip.style.init", function(self, tip)
    ...
    -- unit/item/spell hooks are registered once below via TooltipDataProcessor
...
-- Tooltip content hooks (OnTooltipSetUnit/Item/Spell no longer exist on this client).
-- Only tooltips TinyTooltip has styled (tip.style) are touched.
local function IsStyled(tip)
...
compat.AddTooltipPostCall("Item", "OnTooltipSetItem", function(self)
    if (not IsStyled(self) or not self.GetItem) then return end
    local link = nosecret(select(2, self:GetItem()))
    if (not link) then return end
    LibEvent:trigger("tooltip:item", self, link)
end)

compat.AddTooltipPostCall("Spell", "OnTooltipSetSpell", function(self, data)
    if (not IsStyled(self)) then return end
    local id = data and nosecret(data.id)
    LibEvent:trigger("tooltip:spell", self, id)
end)
