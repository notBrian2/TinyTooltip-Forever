    ...
    frame:SetScript("OnClick", function(self)
        ...
        if (ColorPickerFrame.SetupColorPickerAndShow) then
            local function apply()
                local tipframe = ColorPickerFrame.tipframe
                local r, g, b = ColorPickerFrame:GetColorRGB()
                local a = tipframe.hasopacity and tonumber(format("%.2f", ColorPickerFrame:GetColorAlpha())) or 1
                r = tonumber(format("%.4f",r))
                g = tonumber(format("%.4f",g))
                b = tonumber(format("%.4f",b))
                tipframe:GetNormalTexture():SetVertexColor(safecolor(r,g,b,a))
                if (tipframe.colortype == "hex") then
                    SetVariable(tipframe.keystring, addon:GetHexColor(r,g,b))
                else
                    SetVariable(tipframe.keystring, {r,g,b,a})
                end
                if (tipframe:GetParent().colordropdown) then
                    UIDropDownMenu_SetText(tipframe:GetParent().colordropdown, VIDEO_QUALITY_LABEL6)
                end
            end
            ...
            ColorPickerFrame:SetupColorPickerAndShow({
                ...
                swatchFunc = apply,
                opacityFunc = self.hasopacity and apply or nil,
            })
            return
        end
        local info = {
...
-- ThinBorderTemplate no longer exists; use a 1px backdrop instead
local function ThinBorderFrame()
...
frame.anchor:SetSize(compat.OptionsPanelWidth-64, 1)
...
framePC.anchor:SetSize(compat.OptionsPanelWidth-64, 1)
...
frameNPC.anchor:SetSize(compat.OptionsPanelWidth-64, 1)
...
frameStatusbar.anchor:SetSize(compat.OptionsPanelWidth-64, 1)
...
frameSpell.anchor:SetSize(compat.OptionsPanelWidth-64, 1)
...
frameFont.anchor:SetSize(compat.OptionsPanelWidth-64, 1)
...
frameVariables.anchor:SetSize(compat.OptionsPanelWidth-64, 1)
...
compat.AddOptionsCategory(frame)
compat.AddOptionsCategory(framePCScrollFrame, frame)
compat.AddOptionsCategory(frameNPCScrollFrame, frame)
compat.AddOptionsCategory(frameStatusbar, frame)
compat.AddOptionsCategory(frameSpell, frame)
compat.AddOptionsCategory(frameFont, frame)
compat.AddOptionsCategory(frameVariables, frame)
...
function SlashCmdList.TinyTooltip(msg, editbox)
    if (msg == "reset") then
        BigTipDB = {}
    elseif (msg == "npc") then
        compat.OpenOptionsCategory(frameNPCScrollFrame)
    elseif (msg == "player") then
        compat.OpenOptionsCategory(framePCScrollFrame)
...
frame.tips:SetFont(frame.tips:GetFont(), 12, "")
